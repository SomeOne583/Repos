...
#include "Edge.h"
#include <queue>  //para el recorrido en anchura bfs

// Grafo no dirigido que usa edge ponderado ( edge tiene origen y destino)
...
void Graph<T>::insert(Edge<T> edge) {
        ...
        adjList[indexTarget].push_back(Edge<int>(edge.target, edge.source, edge.weight));
}
